...
snell_by_subgroup <- function(data, subgroup, trait) {
  subgroup_categories <- unique(data[[subgroup]])
  snell_results <- data.frame(matrix(data = NA,
                                     nrow = length(subgroup_categories),
                                     ncol = 5),
                              row.names = subgroup_categories)
  colnames(snell_results) <- c(1, 2, 3, 4, 6)
  
  for (category in subgroup_categories){
    subset <- filter(data, subgroup == category, !is.na(trait))
    if (nrow(subset) < 10){ 
      data[[trait]][data[[subgroup]] == category] <- NA
    }
    else {
      freqtable <- buildfreqtable(data = data, trait = trait) #should probably be data = subset
      snell_scores <- snell(freqtable)
      snell_results[category,] <- snell_scores
    }
  }
  
  snell_results <- snell_results %>%
    rownames_to_column(var = "subgroup") %>%
    pivot_longer(cols = colnames(snell_results),
                 names_to = "OriginalScore",
                 values_to = "CEP1Snell") %>%
    mutate(OriginalScore = as.numeric(OriginalScore))
  
  browser()
  
  data <- inner_join(x = data,
                     y = snell_results,
                     by = c(subgroup, trait = "OriginalScore"))
