---
title: "Test Snell"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)


setwd(dirname(rstudioapi::getActiveDocumentContext()$path))

source("./Functions.R")
```


Create test data (from snell)
```{r}

data <- data.frame(
 "X-3" = c(0,6,0,0,0,2,3,0,1,2,0,5),
 "X-2" = c(0,3,0,4,0,4,4,0,2,2,0,1),
 "X-1" = c(0,1,3,1,0,3,3,1,0,2,0,1),
 "X0" = c(3,0,2,2,0,1,0,1,0,0,0,0),
 "X1" = c(3,1,2,4,2,0,1,1,1,2,4,1),
 "X2" = c(2,1,4,0,5,2,1,5,4,4,1,3),
 "X3" = c(4,0,1,1,5,0,0,4,4,0,7,1),
 row.names = as.character(1:12)
)

data
```





```{r}
source("./Functions.R")
snell(data)
```

My results are slightly different from the published rsults for the outer two categories. I think mine are actually more accurate though since the deflection in the paper is based on a rule of thumb and mine is actually calculated.


Test Tong data
Create Test data from milton
```{r}
tong_cumfreqs <- data.frame(
  "Easy" = c(0,0,1,0,1),
  "Medium" = c(4,7,0,3,1),
  "Difficult" = c(25,44,20,28,15)
)

tong
```




